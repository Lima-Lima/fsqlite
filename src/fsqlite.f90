...
! --- LSL 2020 --- !
! sqlite3_update
subroutine sqlite3_update( db, tablename, columns, eid)
   type(SQLITE_DATABASE)                       :: db
   character(len=*)                            :: tablename
   type(SQLITE_COLUMN), dimension(:), target   :: columns
   character(len=20+80*size(columns))          :: command,precommand
   ...
   !
   ! Prepare the update statement for this table
   !
   write( eid_str, '(I22.1)') eid
   lencol = ubound(columns,dim=1)
   write( precommand, '(100a)' ) 'update ', trim(tablename), ' SET ', &
       (trim(columns(i)%name) // ' = ?, ', i = 1,lencol-1), &
       (trim(columns(lencol)%name) // ' = ? '), 'WHERE experimentid = '
   !write( command, '(100a)' ) 'insert into ', trim(tablename), ' values(', &
   !   ('? = ?,', i = 1,size(columns)-1), '?)'
   write(command ,'(3a)')  trim(precommand), ' ', adjustl(trim(eid_str))
   call stringtoc( command )
   prepared_columns => columns
   call sqlite3_prepare( db, command, stmt, prepared_columns )
